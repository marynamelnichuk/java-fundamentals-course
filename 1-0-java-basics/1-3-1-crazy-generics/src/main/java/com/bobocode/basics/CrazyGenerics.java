...
public class CrazyGenerics {
    ...
    interface ComparableCollection<E> extends Collection<E>, Comparable<Collection<?>> {
        @Override
        default int compareTo(Collection<?> comparableCollection) {
            return Integer.compare(this.size(), comparableCollection.size());
        }
    }
    ...
    static class CollectionUtil {
        ...
        public static boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            return entities.stream().anyMatch(entity -> Objects.isNull(entity.getUuid()));
        }
        ...
        public static <T> Optional<T> findMax(Iterable<T> elements, Comparator<? super T> comparator) {
            ...
            while (iterator.hasNext()) {
                T elemToCompare = iterator.next();
                int comparisonRes = comparator.compare(maxValue, elemToCompare);
                if (comparisonRes <= 0) {
                    maxValue = elemToCompare;
                }
            }
            return Optional.of(maxValue);
            //return elements.stream().max(comparator); //with Stream API
        }
        ...
        private static <T> void swapHelper(List<T> elements, int i, int j) {
            T firstElem = elements.get(i);
            elements.set(i, elements.get(j));
            elements.set(j, firstElem);
        }
    }
}